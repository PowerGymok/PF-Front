...
function mapToBooking(item: any): Booking {
  ...
  return {
    ...
    duration: parseInt(item.class?.duration ?? "0", 10),
...
export default function BookingClient() {
  ...
  // ── Variables derivadas — antes de cualquier return condicional ──
  ...
  // ── Hooks de reservaciones — SIEMPRE antes de returns condicionales ──
  ...
  const onReserve = async (booking: Booking) => {
    console.log(
      "🚀 onReserve llamado",
      booking.id_class_schedule,
      new Date().toISOString(),
    );
    if (isReserving.current) {
      return { success: false, message: "Procesando reserva anterior…" };
    }
    ...
    try {
      ...
      if (result.success) {
        // Update optimista — decrementa cupos disponibles sin refetch
        setRawBookings((prev) =>
          prev.map((b) =>
